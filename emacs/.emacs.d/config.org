...
#+BEGIN_SRC emacs-lisp
    (use-package visual-regexp
      ...
      (use-package visual-regexp-steroids
        :defer nil)
      (require 'visual-regexp-steroids))  ;TODO does that work
#+END_SRC
* beacon
#+BEGIN_SRC emacs-lisp
  (use-package beacon
...
#+BEGIN_SRC emacs-lisp
  (use-package tex-site
    ...
    ;; only fold environments and macro, not maths
    (setq TeX-fold-type-list '(env macro))

    ;; make preview images bigger
    (setq preview-scale-function 2.1)
    ...
    ;; stolen from https://yoo2080.wordpress.com/2016/11/15/how-to-make-prettify-symbols-mode-work-with-auctex/
    ;; ran after fontification
    (defun my/delayed-latex ()
      (run-with-idle-timer 0 nil (lambda ()
                                 (rainbow-delimiters-mode 1)
                                 (hl-todo-mode 1)
                                 (TeX-fold-buffer))))
    (add-hook 'TeX-mode-hook #'my/delayed-latex)

    ;; stolen from https://tex.stackexchange.com/questions/111726/preview-tikz-figure-in-auctex/111940#111940
    (eval-after-load "preview"
      '(progn (add-to-list 'preview-default-preamble "\\PreviewEnvironment{tikzpicture}" t)
              (add-to-list 'preview-default-preamble "\\PreviewEnvironment{algorithm}" t))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package yasnippet
    ...
    (with-eval-after-load 'company
      ...
      (defvar company-mode/enable-yas ()   ; disable it for now
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package color-theme-solarized
    :defer nil
    :disabled
    :config (load-theme 'solarized 'noconfirm))
#+END_SRC
** cursor
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  ("w" (lambda ()
      ...
      ("<return>" nil)))
#+END_SRC
** disable other themes before loading a new one
#+BEGIN_SRC emacs-lisp
  ;; stolen from https://www.reddit.com/r/emacs/comments/8v9lgu/emacs_theme_configuration_is_very_confusing/e1lpcpk
...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    (evil-define-key my/all-evil-states markdown-mode-map (kbd "<tab>") #'markdown-cycle)
